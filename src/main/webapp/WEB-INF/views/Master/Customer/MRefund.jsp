							...
							<tr>
								...
								<c:choose>
									<c:when test="${refund.getrCheck() eq '1'}">
										<td><input type="submit" class="btn" value="환불완료"
											style="height: 30px; position: relative; top: -8px; color: red;"></td>
									</c:when>
									<c:when test="${refund.getrCheck() ne '1'}">
										<td><input type="submit" class="btn" value="환불신청"
											style="height: 30px; position: relative; top: -8px; color: #004d99;"></td>
									</c:when>
								</c:choose>
							</tr>
						</form>
		...
		<ul class="pagination justify-content-center" style="position:relative; top:-80px;">
			...
			<c:choose>
				<c:when test="${Paging.pageNo eq Paging.firstPageNo }">
					<li class="page-item disabled"><a class="page-link"
						href="MRefund?page=${Paging.prevPageNo}">Previus</a></li>
				</c:when>
				<c:otherwise>
			...
			<c:forEach var="i" begin="${Paging.startPageNo }"
				...
				<c:choose>
					<c:when test="${i eq Paging.pageNo }">
						<li class="page-item disabled"><a class="page-link"
							href="MRefund?page=${i}"><c:out value="${i}" /></a></li>
					</c:when>
					<c:otherwise>
						<li class="page-item"><a class="page-link"
							href="MRefund?page=${i}"><c:out value="${i}" /></a></li>
			...
			<c:choose>
				<c:when test="${Paging.pageNo eq Paging.finalPageNo }">
					<li class="page-item disabled"><a class="page-link"
						href="MRefund?page=${Paging.nextPageNo}">Next</a></li>
				</c:when>
				<c:otherwise>
					<li class="page-item"><a class="page-link"
						href="MRefund?page=${Paging.nextPageNo}">Next</a></li>
